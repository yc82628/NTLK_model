...
def distilbert_predict_with_confidence(trainer, texts: pd.Series, tokenizer,
                                       batch_size: int = 64):
    """
    Return (predicted_label, confidence, prob_positive).

    - predicted_label : 0/1 argmax prediction
    - confidence      : probability of the WINNING class (used for the
                        confidence-filtered divergence metric)
    - prob_positive   : probability of class 1 = "Recommended". This is the
                        score ROC-AUC and precision-recall curves need; using
                        `confidence` there would be wrong, because it collapses
                        both classes onto one scale.

    Computed in mini-batches with per-batch padding to keep memory flat.
    """
    model = trainer.model
    model.eval()
    texts = list(texts)
    ...
    for i in range(0, len(texts), batch_size):
        ...
        pos.append(probs[:, 1].detach().cpu().numpy())
    return np.concatenate(preds), np.concatenate(confs), np.concatenate(pos)