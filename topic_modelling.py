...
MEMORY NOTES
------------
- BERTopic runs with calculate_probabilities=False (the dense n_docs x n_topics
  membership matrix is the slowest, most memory-hungry step at 50k docs and the
  usual "stuck forever" symptom). Per-document confidence comes from HDBSCAN's
  cheap 1-D membership strength (probabilities_) instead.
- Sentence embeddings are computed ONCE for the whole corpus and reused across
  the stability runs + the final run (was one full re-embedding per run).

COHERENCE NOTE
--------------
BERTopic extracts its top words from bert_text (Stream B: minimal cleaning),
but the coherence dictionary is built from clean_text (Stream A: lowercased,
lemmatised, stopwords removed). Any topic word not present in the Stream A
dictionary makes gensim raise "unable to interpret topic as ... ids". We
therefore filter each topic's words to those present in the dictionary before
scoring. See the write-up caveat in the chat if the LDA-vs-BERTopic coherence
comparison is a headline result.
"""
...
def run_lda(df: pd.DataFrame):
    ...
    k_values = range(config.LDA_MIN_K, config.LDA_MAX_K + 1,
                     getattr(config, "LDA_K_STEP", 1))
    for k in k_values:
