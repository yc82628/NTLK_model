...
def _cache_key() -> str:
    """
    Fingerprint of every setting that changes the sampled data. If any of these
    change, the key changes and the cache rebuilds automatically — so a stale
    cache can never silently feed wrong data into the pipeline.
    """
    parts = [
        config.SAMPLE_SIZE, config.MIN_REVIEW_LEN, config.ENGLISH_ONLY,
        config.RANDOM_STATE, config.POOL_CAP,
    ]
    raw = "|".join(str(p) for p in parts)
    return hashlib.md5(raw.encode()).hexdigest()[:10]


def _cache_path(stage: str) -> str:
    return os.path.join(config.CACHE_DIR, f"{stage}_{_cache_key()}.pkl")
...
def _load_cache(stage: str):
    ...
    if config.USE_CACHE and os.path.exists(path):
        try:
            ...
            print(f"[CACHE] Loaded {stage} from {path} ({len(df):,} rows) "
                  f"— skipping the slow CSV pass")
            return df
        except Exception as e:
            print(f"[CACHE] Could not read {path} ({e}); rebuilding.")
    return None


def _save_cache(df: pd.DataFrame, stage: str):
    ...
    try:
        df.to_pickle(path)
        print(f"[CACHE] Saved {stage} to {path} — later runs will reuse this")
    except Exception as e:
        print(f"[CACHE] Could not write {path} ({e}); continuing without cache.")
...
def preprocess(df: pd.DataFrame) -> pd.DataFrame:
    """Apply both preprocessing streams to the dataframe (cached)."""
    cached = _load_cache("preprocessed")
    if cached is not None:
        return cached
