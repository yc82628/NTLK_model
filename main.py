...
def main():
    ...
    (test_df["distilbert_pred"], test_df["distilbert_confidence"],
     test_df["distilbert_prob_positive"]) = (
    ...
    # Threshold-independent comparison. VADER is scored by its raw compound
    # polarity and DistilBERT by its probability of "recommended", so neither
    # depends on a chosen cut-off.
    score_dict = {
        "VADER (compound score)": test_df["vader_compound"].values,
        "DistilBERT (P of recommended)": test_df["distilbert_prob_positive"].values,
    }
    auc_results = evaluation.save_roc_curves(test_df[config.LABEL_COL], score_dict)
    ap_results = evaluation.save_pr_curves(test_df[config.LABEL_COL], score_dict)

    pd.DataFrame([
        {"model": k, "roc_auc": auc_results[k], "average_precision": ap_results[k]}
        for k in auc_results
    ]).to_csv(f"{config.TABLE_DIR}/ranking_metrics.csv", index=False)
    ...
    # Readable heatmap: top-volume topics only, low-count cells suppressed
    evaluation.save_divergence_heatmap_readable(
        df, topic_col="bertopic_topic",
        fname="divergence_heatmap.png",
        title="Sentiment vs Rating Disagreement, by Genre and Topic",
    )
    # Plain bar chart — the most readable summary for non-technical readers
    evaluation.save_divergence_by_genre_bar(div_table)

    # ── Step 9: Bot / adversarial detection ──────────────────────────────
    print("\n" + "=" * 60)
    print("STEP 9 — REVIEW AUTHENTICITY SCREENING")
    print("=" * 60)
    df = bot_detection.extract_bot_features(df)
    flag_table = bot_detection.summarise_provenance_flags(df)
    flag_table.to_csv(f"{config.TABLE_DIR}/authenticity_flags.csv", index=False)
    df = bot_detection.run_bot_detection(df)
    evaluation.save_authenticity_flag_chart(flag_table)
